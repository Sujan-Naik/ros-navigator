...
from second_coursework.msg import RoomCheckAction, RoomCheckFeedback, RoomCheckResult
from second_coursework.srv import robot_move, yolo_detect


class ITRStateMachine():
    def __init__(self):
        ...
        asw = ActionServerWrapper(
            'robot_state_machine_action_server', RoomCheckAction,
        ...
        if robot_move_proxy(room_name).reached:
            ...
            while rospy.get_rostime().to_time() - reached_time < navigation_time:
                ...
                try:
                    ...
                    rospy.logerr_once('Error detecting frames with YOLO')
                robot_move_proxy(room_name)

            return 'succeeded'
        return 'aborted'
    except rospy.ServiceException as e:
        rospy.logerr_once('Error sending robot move servicce')
